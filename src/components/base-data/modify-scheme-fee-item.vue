    ...
    <i-form :label-width="100" :model="model" :rules="rules" ref="form" inline>
      ...
      <i-form-item label="费用类型">
        <RadioGroup v-model="costTypes">
          <Radio label="1">收取总额比例</Radio>
          <Radio label="2">固定费用</Radio>
        </RadioGroup>
      </i-form-item>
      <i-form-item label="收取总额比例" prop="repayProportion" v-if="costTypes===`1`">
...
export default class ModifySchemeFeeItem extends Vue {
  ...
  private revertData() {
    ...
    // 费用类型
    this.costTypes = this.model.repayProportion ? "1" : "2";
  }
